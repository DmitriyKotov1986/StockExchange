...
/// class Binance
///
Binance::Binance(const StockExchange::StockExchangeConfig& config, const Common::HTTPSSLQuery::ProxyList& proxyList, QObject *parent)
...
void Binance::getAnswerHTTP(const QByteArray &answer, quint64 id)
{
...
void Binance::sendLogMsgHTTP(Common::TDBLoger::MSG_CODE category, const QString &msg, quint64 id)
{
    emit sendLogMsg(STOCK_ID, category, QString("HTTP request %1: %2").arg(id).arg(msg));
}

void Binance::getKLinesPool(const TradingCatCommon::PKLinesList &klines)
{
...
void Binance::errorOccurredPool(Common::EXIT_CODE errorCode, const QString &errorString)
{
    emit errorOccurred(STOCK_ID, errorCode, QString("KLines Pool error: %1").arg(errorString));
}

void Binance::sendLogMsgPool(Common::TDBLoger::MSG_CODE category, const QString &msg)
{
...
void Binance::sendUpdateMoney()
{
    ...
    QUrlQuery query;
    query.addQueryItem("permissions", "SPOT");
    query.addQueryItem("symbolStatus", "TRADING");

    QUrl url(*BASE_URL);
    url.setPath("/api/v3/exchangeInfo");
    url.setQuery(query);
...
void Binance::parseMoney(const QByteArray &answer)
{
    ...
    try
    {
        ...
        {
            const auto moneyDataJson = JSONReadMap(symbolDataValueJson, "Root/symbols/[]");
            const auto moneyName = JSONReadMapString(moneyDataJson, "symbol", "Root/symbols/[]", false);
...
*/
    ...
    for (const auto& klineId: *klinesIdList)
    {
        ...
        {
            auto kline = std::make_unique<BinanceKLine>(klineId, QDateTime::currentDateTime().addMSecs(-static_cast<qint64>(klineId.type) * KLINES_COUNT_HISTORY));
