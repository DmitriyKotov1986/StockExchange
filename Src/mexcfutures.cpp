...
#include "StockExchange/mexcklinefutures.h"
...
const StockExchangeID MexcFutures::STOCK_ID("MEXC_FUTURES");

///////////////////////////////////////////////////////////////////////////////
/// class MexcFutures
///
MexcFutures::MexcFutures(const StockExchange::StockExchangeConfig& config, const Common::HTTPSSLQuery::ProxyList& proxyList, QObject *parent)
    ...
    , _proxyList(proxyList)
{
    ...
    {
        _headers.insert(QByteArray{"X-MexcFutures-APIKEY"}, _config.user.toUtf8());
    }
    //
...
void MexcFutures::getAnswerHTTP(const QByteArray &answer, quint64 id)
{
...
void MexcFutures::errorOccurredHTTP(QNetworkReply::NetworkError code, quint64 serverCode, const QString &msg, quint64 id, const QByteArray& answer)
{
    Q_UNUSED(code);
    Q_UNUSED(serverCode);
...
void MexcFutures::sendLogMsgHTTP(Common::TDBLoger::MSG_CODE category, const QString &msg, quint64 id)
{
    emit sendLogMsg(STOCK_ID, category, QString("HTTP request %1: %2").arg(id).arg(msg));
}

void MexcFutures::getKLinesPool(const TradingCatCommon::PKLinesList &klines)
{
...
void MexcFutures::errorOccurredPool(Common::EXIT_CODE errorCode, const QString &errorString)
{
    emit errorOccurred(STOCK_ID, errorCode, QString("KLines Pool error: %1").arg(errorString));
}

void MexcFutures::sendLogMsgPool(Common::TDBLoger::MSG_CODE category, const QString &msg)
{
    emit sendLogMsg(STOCK_ID, category, QString("KLines Pool: %1").arg(msg));
}


void MexcFutures::sendUpdateMoney()
{
    Q_CHECK_PTR(_http);
    ...
    QUrl url(*BASE_URL);
    url.setPath("/api/v1/contract/detail");

    _currentRequestId = _http->send(url, Common::HTTPSSLQuery::RequestType::GET);
}

void MexcFutures::restartUpdateMoney()
{
...
void MexcFutures::parseMoney(const QByteArray &answer)
{
    ...
    try
    {
        ...
        const auto moneyListJson = JSONReadMapToArray(answerJson, "data", "Root/Data/[]");
        for (const auto& moneyDataJson: moneyListJson)
        {
            const auto moneyData = JSONReadMap(moneyDataJson, "Root/Data/[Data]");
            const auto moneyName = JSONReadMapString(moneyData, "symbol", "Root/Data/[]/symbol", false);
    ...
    emit sendLogMsg(STOCK_ID, TDBLoger::MSG_CODE::INFORMATION_CODE, QString("The earch for the list of moneylist complite successfully"));
...
void MexcFutures::makeKLines(const TradingCatCommon::PKLinesIDList klinesIdList)
{
...
*/
    ...
    for (const auto& klineId: *klinesIdList)
    {
        ...
        {
            auto kline = std::make_unique<MexcKLineFutures>(klineId,
                                                     QDateTime::currentDateTime().addMSecs(-static_cast<qint64>(klineId.type) * KLINES_COUNT_HISTORY),
                                                     _config.password);
