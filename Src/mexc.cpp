...
#include "StockExchange/mexckline.h"
...
const StockExchangeID Mexc::STOCK_ID("MEXC");

///////////////////////////////////////////////////////////////////////////////
/// class Mexc
///
Mexc::Mexc(const StockExchange::StockExchangeConfig& config, const Common::HTTPSSLQuery::ProxyList& proxyList, QObject *parent)
    ...
    , _proxyList(proxyList)
{
    ...
    {
        _headers.insert(QByteArray{"X-MEXC-APIKEY"}, _config.user.toUtf8());
    }
    //
...
void Mexc::errorOccurredHTTP(QNetworkReply::NetworkError code, quint64 serverCode, const QString &msg, quint64 id, const QByteArray& answer)
{
    Q_UNUSED(code);
    Q_UNUSED(serverCode);
...
void Mexc::sendLogMsgHTTP(Common::TDBLoger::MSG_CODE category, const QString &msg, quint64 id)
{
    emit sendLogMsg(STOCK_ID, category, QString("HTTP request %1: %2").arg(id).arg(msg));
}

void Mexc::getKLinesPool(const TradingCatCommon::PKLinesList &klines)
{
...
void Mexc::errorOccurredPool(Common::EXIT_CODE errorCode, const QString &errorString)
{
    emit errorOccurred(STOCK_ID, errorCode, QString("KLines Pool error: %1").arg(errorString));
}

void Mexc::sendLogMsgPool(Common::TDBLoger::MSG_CODE category, const QString &msg)
{
    emit sendLogMsg(STOCK_ID, category, QString("KLines Pool: %1").arg(msg));
}


void Mexc::sendUpdateMoney()
{
    Q_CHECK_PTR(_http);
...
void Mexc::parseMoney(const QByteArray &answer)
{
    ...
    try
    {
        ...
        const auto docJson = JSONParseToMap(answer);
        const auto moneyListJson = JSONReadMapToArray(docJson, "data", "Root/[Data]");
        for (const auto& moneyNameJson: moneyListJson)
        {
            const auto moneyName = JSONReadString(moneyNameJson, "Root/[Data]", false);
...
void Mexc::makeKLines(const TradingCatCommon::PKLinesIDList klinesIdList)
{
...
*/
    ...
    for (const auto& klineId: *klinesIdList)
    {
        ...
        {
            auto kline = std::make_unique<MexcKLine>(klineId,
