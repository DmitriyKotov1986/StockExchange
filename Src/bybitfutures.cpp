...
#include "StockExchange/bybitklinefutures.h"
...
const StockExchangeID BybitFutures::STOCK_ID("BYBIT_FUTURES");

///////////////////////////////////////////////////////////////////////////////
/// class BybitFutures
///
BybitFutures::BybitFutures(const StockExchange::StockExchangeConfig& config, const Common::HTTPSSLQuery::ProxyList& proxyList, QObject *parent)
...
void BybitFutures::getAnswerHTTP(const QByteArray &answer, quint64 id)
{
...
void BybitFutures::errorOccurredHTTP(QNetworkReply::NetworkError code, quint64 serverCode, const QString &msg, quint64 id, const QByteArray& answer)
{
    Q_UNUSED(code);
    Q_UNUSED(serverCode);
...
void BybitFutures::sendLogMsgHTTP(Common::TDBLoger::MSG_CODE category, const QString &msg, quint64 id)
{
    emit sendLogMsg(STOCK_ID, category, QString("HTTP request %1: %2").arg(id).arg(msg));
}

void BybitFutures::getKLinesPool(const TradingCatCommon::PKLinesList &klines)
{
...
void BybitFutures::errorOccurredPool(Common::EXIT_CODE errorCode, const QString &errorString)
{
    emit errorOccurred(STOCK_ID, errorCode, QString("KLines Pool error: %1").arg(errorString));
}

void BybitFutures::sendLogMsgPool(Common::TDBLoger::MSG_CODE category, const QString &msg)
{
    emit sendLogMsg(STOCK_ID, category, QString("KLines Pool: %1").arg(msg));
}


void BybitFutures::sendUpdateMoney()
{
    Q_CHECK_PTR(_http);
...
void BybitFutures::restartUpdateMoney()
{
...
void BybitFutures::parseMoney(const QByteArray &answer)
{
    ...
    try
    {
        ...
        for (const auto& moneyDataValueJson: moneyListJson)
        {
            ...
            const auto contractType = JSONReadMapString(moneyDataJson, "contractType", "Root/result/list/[]/contractType", false);
...
void BybitFutures::makeKLines(const TradingCatCommon::PKLinesIDList klinesIdList)
{
...
*/
    ...
    for (const auto& klineId: *klinesIdList)
    {
        ...
        {
            auto kline = std::make_unique<BybitKLineFutures>(klineId, QDateTime::currentDateTime().addMSecs(-static_cast<qint64>(klineId.type) * KLINES_COUNT_HISTORY));;
