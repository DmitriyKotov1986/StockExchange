...
#include "StockExchange/kucoinkline.h"
...
/// class Kucoin
///
Kucoin::Kucoin(const StockExchange::StockExchangeConfig& config, const Common::ProxyList& proxyList, QObject *parent)
...
void Kucoin::errorOccurredHTTP(QNetworkReply::NetworkError code, quint64 serverCode, const QString &msg, quint64 id, const QByteArray& answer)
{
    Q_UNUSED(code);
    Q_UNUSED(serverCode);
...
void Kucoin::sendLogMsgHTTP(Common::MSG_CODE category, const QString &msg, quint64 id)
{
...
void Kucoin::sendLogMsgPool(Common::MSG_CODE category, const QString &msg)
{
...
void Kucoin::parseMoney(const QByteArray &answer)
{
    ...
    try
    {
        ...
        const auto moneyDataArrayJson = JSONReadMapToArray(rootJson, "data", "Root/data");

        for (const auto& moneyDataValueJson: moneyDataArrayJson)
        {

            const auto moneyDataJson = JSONReadMap(moneyDataValueJson, "Root/data/[]");
            const auto moneyName =JSONReadMapString(moneyDataJson, "symbol", "Root/data/[]/symbol", false);
...
void Kucoin::makeKLines(const TradingCatCommon::PKLinesIDList klinesIdList)
{
