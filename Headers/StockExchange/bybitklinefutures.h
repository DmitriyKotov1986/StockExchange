...
class BybitKLineFutures
    : public TradingCatCommon::IKLine
{
    Q_OBJECT

public:
    BybitKLineFutures(const TradingCatCommon::KLineID& id, const QDateTime& lastClose, QObject* parent = nullptr);
    ~BybitKLineFutures() override = default;

    void start() override;
    void stop() override;

private:
    BybitKLineFutures() = delete;
    Q_DISABLE_COPY_MOVE(BybitKLineFutures);
