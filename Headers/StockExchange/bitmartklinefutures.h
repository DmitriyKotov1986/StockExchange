...
class BitmartKLineFutures
    : public TradingCatCommon::IKLine
{
    Q_OBJECT

public:
    BitmartKLineFutures(const TradingCatCommon::KLineID& id, const QDateTime& lastClose, QObject* parent = nullptr);
    ~BitmartKLineFutures() override = default;

    void start() override;
    void stop() override;

private:
    BitmartKLineFutures() = delete;
    Q_DISABLE_COPY_MOVE(BitmartKLineFutures);
